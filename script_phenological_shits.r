...
liste=unique(donnf$Species_mode) # define the speciesand mode liste

res=as.data.frame(matrix(NA,1,7))
names(res)=c("Species_mode","Year_effect","Year_error","Year_pval","Year_var_effect","Year_var_error","Year_var_pval")

for (i in 1:length(liste)){
tab=subset(donnf,Species_gen==liste[i])
...
if(liste[i]=="Endochironomus albipennis_1" | liste[i]=="Endochironomus albipennis_2" | liste[i]=="Psodos quadrifaria_NA" | liste[i]=="Rhyacia helvetina_NA" |
liste[i]=="Bubas bison_NA" | liste[i]=="Colias tyche_NA" | liste[i]=="Rheumaptera cervinalis_NA" | liste[i]=="Eudonia angustea_2" |
liste[i]=="Onthophagus emarginatus_NA" | liste[i]=="Tanypus kraatzi_NA" | liste[i]=="Lithophane lamda_NA" | liste[i]=="Euonthophagus amyntas_NA"){
